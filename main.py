...
# Создаем окно
root = p.display.set_mode((1500, 1000))

# 2х мерный список с помощью генераторных выражений
cells = [
    [random.choice([0, 1]) for j in range(root.get_width() // 6)]
    for i in range(root.get_height() // 6)
]
...
while 1:
    ...
    for i in range(0, len(cells)):
        for j in range(0, len(cells[i])):
            p.draw.rect(root, (255 * cells[i][j] % 256, 0, 0), [i * 10, j * 10, 10, 10])
